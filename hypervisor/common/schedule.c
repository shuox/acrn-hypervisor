...
bool is_idle_thread(struct thread_object *obj)
{
	uint16_t pcpu_id = obj->pcpu_id;
	return (obj == &per_cpu(idle, pcpu_id));
}

static inline bool is_blocked(struct thread_object *obj)
{
	return obj->status == THREAD_STS_BLOCKED;
}

static inline bool is_runnable(struct thread_object *obj)
{
	return obj->status == THREAD_STS_RUNNABLE;
}

static inline bool is_running(struct thread_object *obj)
{
	return obj->status == THREAD_STS_RUNNING;
}

static inline void set_thread_status(struct thread_object *obj, uint16_t status)
{
	obj->status = status;
}
