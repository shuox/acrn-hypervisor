...
static int32_t shell_list_vcpu(__unused int32_t argc, __unused char **argv)
{
	...
	char state[32], thread_status[32];
	uint16_t i;
	uint16_t idx;

	shell_puts("\r\nVM ID    PCPU ID    VCPU ID    VCPU ROLE    VCPU STATE    THREAD STATUS"
		"\r\n=====    =======    =======    =========    ==========    ===========\r\n");
	...
	for (idx = 0U; idx < CONFIG_MAX_VM_NUM; idx++) {
		...
		foreach_vcpu(i, vm, vcpu) {
			...
			case THREAD_STS_RUNNING:
				(void)strncpy_s(thread_status, 32U, "RUNNING", 32U);
				break;
			case THREAD_STS_RUNNABLE:
				(void)strncpy_s(thread_status, 32U, "RUNNABLE", 32U);
				break;
			case THREAD_STS_BLOCKED:
				(void)strncpy_s(thread_status, 32U, "BLOCKED", 32U);
				break;
			default:
				(void)strncpy_s(thread_status, 32U, "Unknown", 32U);
			}
